...
class Feat extends React.Component {
    ...
    approveFeat = async () => {
        try {
            ...
            const error = _.get(exception, 'request.data.error');
            this.setState({ loading: false, loadingActive: true, message: error ? error : 'Godkännandet misslyckades, pröva igen' });
        }
    };
    ...
    render() {
        ...
        return (
            <div>
                <Dialog fullScreen maxWidth="md" open={this.state.open} onClose={this.props.clearClickedFeat()} TransitionComponent={this.transition} transitionDuration={1000}>
                    <DialogContent>
                        {feat &&
                            ...
                            <Grid direction="row" container spacing={8} style={{ display: 'flex' }}>
                                ...
                                <Grid item xs={featColumnWidth}>
                                    <List dense={false}>
                                        ...
                                        <ListItem>
                                            <ListItemText primary="Tid" secondary={moment.unix(feat.date).format('DD.MM.YYYY HH:mm:ss')}/>
                                        </ListItem>
                                        <Divider />
                                        <ListItem>
                        ...
                        {feat &&
                            <div>
                                ...
                                {_.get(user, 'type') === 'admin' &&
                                    <Button style={{ margin: '5px' }} label="Radera" variant="contained" color="primary" onClick={this.deleteFeat}>Radera</Button>}
                                {_.get(user, 'type') === 'admin' &&
                                    <Button variant="contained" color="primary" onClick={() => this.setState({ editMode: true })}>Editera</Button>}
                                {_.get(user, 'type') === 'admin' && !feat.approved &&
                                    <Button style={{ margin: '5px' }} label="Godkänn" variant="contained" color="primary" onClick={this.approveFeat}>Godkänn</Button>}
                            </div>}
                    </DialogActions>
                </Dialog>
